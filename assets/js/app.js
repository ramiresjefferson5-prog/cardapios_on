...
/* =========================================================
   ADMIN ERP V8 - Navegação robusta dos editores
   Corrige o "modal de edição" que ficava sem rota clara de volta
   ========================================================= */
(function adminV8EditorNavigation(){
    const $ = (id) => document.getElementById(id);
    const panel = () => $('modal-admin-panel');
    const editorIds = ['admin-produto-form-card', 'admin-promocao-form-card', 'admin-taxa-form-card'];

    const meta = {
        produto: {
            formId: 'admin-produto-form-card',
            titleId: 'admin-produto-form-title',
            create: 'Novo produto',
            edit: 'Editar produto',
            subtitle: 'Cardápio / Produto'
        },
        promocao: {
            formId: 'admin-promocao-form-card',
            titleId: 'admin-promocao-form-title',
            create: 'Nova promoção',
            edit: 'Editar promoção',
            subtitle: 'Comercial / Promoção'
        },
        taxa: {
            formId: 'admin-taxa-form-card',
            titleId: 'admin-taxa-form-title',
            create: 'Novo bairro',
            edit: 'Editar taxa',
            subtitle: 'Logística / Entrega'
        }
    };

    function getFormKind(form) {
        if (!form) return '';
        if (form.id.includes('produto')) return 'produto';
        if (form.id.includes('promocao')) return 'promocao';
        if (form.id.includes('taxa')) return 'taxa';
        return '';
    }

    function getKindMeta(kind) {
        return meta[kind] || meta.produto;
    }

    function ensureEditorShell(form, kind) {
        if (!form || form.dataset.v8EditorReady === 'true') return;

        const kindMeta = getKindMeta(kind);
        const toolbar = document.createElement('div');
        toolbar.className = 'admin-editor-toolbar';
        toolbar.innerHTML = `
            <button type="button" class="admin-editor-back" aria-label="Voltar" data-admin-editor-close="true">
                <i class="fas fa-arrow-left"></i>
            </button>
            <div class="admin-editor-heading">
                <small>${kindMeta.subtitle}</small>
                <strong>${kindMeta.create}</strong>
            </div>
            <button type="button" class="admin-editor-save-top" data-admin-editor-save="true">
                <i class="fas fa-save"></i><span>Salvar</span>
            </button>
        `;

        const body = document.createElement('div');
        body.className = 'admin-editor-body custom-scroll';

        const children = Array.from(form.childNodes);
        const actionBar = form.querySelector('.erp-form-actions');

        children.forEach(node => {
            if (node === actionBar) return;
            body.appendChild(node);
        });

        form.insertBefore(toolbar, form.firstChild);
        form.insertBefore(body, actionBar || null);
        if (actionBar && actionBar.parentElement !== form) form.appendChild(actionBar);

        const closeBtn = toolbar.querySelector('[data-admin-editor-close]');
        const saveBtn = toolbar.querySelector('[data-admin-editor-save]');
        ...
        if (saveBtn) {
            saveBtn.addEventListener('click', function(ev) {
                ...
                if (typeof form.requestSubmit === 'function') form.requestSubmit();
                    ...
                    const submit = form.querySelector('.erp-form-actions .admin-save-btn, button[type="submit"]');
        ...
        form.addEventListener('click', ev => ev.stopPropagation());
        form.dataset.v8EditorReady = 'true';
    }

    function setToolbarText(kind, mode) {
        const m = getKindMeta(kind);
        const form = $(m.formId);
        if (!form) return;
        const heading = form.querySelector('.admin-editor-heading');
        if (!heading) return;
        const title = mode === 'edit' ? m.edit : m.create;
        const small = heading.querySelector('small');
        const strong = heading.querySelector('strong');
        if (small) small.textContent = m.subtitle;
        if (strong) strong.textContent = title;
    }

    function prepareAllEditors() {
        Object.keys(meta).forEach(kind => {
            const m = meta[kind];
            const form = $(m.formId);
            if (form) ensureEditorShell(form, kind);
        });
    }
    ...
    function closeCurrentEditor() {
        editorIds.forEach(id => {
            ...
            if (!el) return;
            el.classList.add('hidden');
            const body = el.querySelector('.admin-editor-body');
        ...
        if (p) {
            p.classList.remove(
                'admin-editor-open',
                'admin-editor-produto',
                'admin-editor-promocao',
                'admin-editor-taxa'
            );
            p.removeAttribute('data-editor-kind');
        }
    }

    window.adminCloseCurrentEditor = function(event) {
        if (event) {
            event.preventDefault();
    ...
    function openEditor(kind, mode) {
        ...
        editorIds.forEach(id => {
            if (id !== m.formId) $(id)?.classList.add('hidden');
        });

        form.classList.add('admin-editor-panel');
        form.classList.remove('hidden');
        p.classList.add('admin-editor-open', 'admin-editor-' + kind);
        p.setAttribute('data-editor-kind', kind);
        setToolbarText(kind, mode || 'create');

        const body = form.querySelector('.admin-editor-body');
        if (body) body.scrollTo({ top: 0, behavior: 'instant' });

        setTimeout(() => {
            const first = form.querySelector('.admin-editor-body input:not([type="hidden"]):not([type="file"]), .admin-editor-body textarea, .admin-editor-body select');
            if (first && window.matchMedia('(min-width: 1025px)').matches) first.focus({ preventScroll: true });
        }, 60);
    }

    function resetProdutoFormV8() {
        const fields = [
            'admin-produto-id',
            'admin-produto-nome',
            'admin-produto-descricao',
            'admin-produto-categoria',
            'admin-produto-preco',
            'admin-produto-preco-promocional',
            'admin-produto-imagem-url',
            'admin-produto-estoque',
            'admin-produto-estoque-minimo'
        ];
        fields.forEach(id => { const el = $(id); if (el) el.value = ''; });
        const file = $('admin-produto-imagem'); if (file) file.value = '';
        const ativo = $('admin-produto-ativo'); if (ativo) ativo.checked = true;
    ...
    function resetPromocaoFormV8() {
        const fields = [
            'admin-promocao-id',
            'admin-promocao-titulo',
            'admin-promocao-descricao',
            'admin-promocao-preco-original',
            'admin-promocao-preco',
            'admin-promocao-imagem-url',
            'admin-promocao-inicio',
            'admin-promocao-fim'
        ];
        fields.forEach(id => { const el = $(id); if (el) el.value = ''; });
        const produto = $('admin-promocao-produto'); if (produto) produto.value = '';
        const file = $('admin-promocao-imagem'); if (file) file.value = '';
        const ativo = $('admin-promocao-ativo'); if (ativo) ativo.checked = true;
        const destaque = $('admin-promocao-destaque'); if (destaque) destaque.checked = true;
        const title = $('admin-promocao-form-title'); if (title) title.textContent = 'Criar promoção';
    ...
    window.editarProdutoAdmin = function(id) {
        ...
        setValue('admin-produto-id', p.id || '');
        setValue('admin-produto-nome', p.nome || '');
        setValue('admin-produto-descricao', p.descricao || '');
        setValue('admin-produto-categoria', p.categoria || '');
        setValue('admin-produto-preco', p.preco ?? '');
        setValue('admin-produto-preco-promocional', p.preco_promocional ?? '');
        setValue('admin-produto-imagem-url', getImagemProduto(p) || '');
        setValue('admin-produto-estoque', p.estoque_atual ?? '');
        setValue('admin-produto-estoque-minimo', p.estoque_minimo ?? '');
        ...
        const controla = $('admin-produto-controla-estoque'); if (controla) controla.checked = !!p.controla_estoque;
        const title = $('admin-produto-form-title'); if (title) title.textContent = 'Editar produto';
        if (typeof window.adminV5SetUpload === 'function') window.adminV5SetUpload('produto', getImagemProduto(p) || '', getImagemProduto(p) ? 'Imagem atual' : '');
        openEditor('produto', 'edit');
    };
    ...
    window.editarPromocaoAdmin = function(id) {
        ...
        preencherSelectProdutosPromocao(adminProdutosCache || []);
        setValue('admin-promocao-id', p.id || '');
        setValue('admin-promocao-titulo', p.titulo || '');
        setValue('admin-promocao-descricao', p.descricao || '');
        setValue('admin-promocao-produto', p.produto_id || '');
        setValue('admin-promocao-preco-original', p.preco_original ?? '');
        setValue('admin-promocao-preco', p.preco_promocional ?? p.preco ?? '');
        setValue('admin-promocao-imagem-url', p.imagem_url || '');
        setValue('admin-promocao-inicio', p.inicia_em ? String(p.inicia_em).slice(0,16) : '');
        setValue('admin-promocao-fim', p.termina_em ? String(p.termina_em).slice(0,16) : '');

        const file = $('admin-promocao-imagem'); if (file) file.value = '';
        const ativo = $('admin-promocao-ativo'); if (ativo) ativo.checked = p.ativo !== false;
        const destaque = $('admin-promocao-destaque'); if (destaque) destaque.checked = p.destaque !== false;
        const title = $('admin-promocao-form-title'); if (title) title.textContent = 'Editar promoção';
        if (typeof window.adminV5SetUpload === 'function') window.adminV5SetUpload('promocao', p.imagem_url || '', p.imagem_url ? 'Imagem atual' : '');
        openEditor('promocao', 'edit');
    };
    ...
    window.editarTaxaAdmin = function(id) {
        ...
        setValue('nova-taxa-id', t.id || '');
        setValue('nova-taxa-bairro', t.bairro || '');
        setValue('nova-taxa-valor', t.taxa ?? t.valor ?? '');
        const title = $('admin-taxa-form-title'); if (title) title.textContent = 'Editar taxa de entrega';
        openEditor('taxa', 'edit');
    };

    const baseMudarAbaV8 = window.mudarAbaAdmin;
    window.mudarAbaAdmin = function(abaId, btnElement) {
        closeCurrentEditor();
        return baseMudarAbaV8 ? baseMudarAbaV8.apply(this, arguments) : undefined;
    };

    const baseFecharModalV8 = window.fecharModal;
    if (typeof baseFecharModalV8 === 'function' && !baseFecharModalV8.__adminV8Wrapped) {
        const wrapped = function(id) {
            if (id === 'modal-admin-panel') closeCurrentEditor();
            return baseFecharModalV8.apply(this, arguments);
        };
        wrapped.__adminV8Wrapped = true;
        window.fecharModal = wrapped;
    }
    ...
    document.addEventListener('DOMContentLoaded', prepareAllEditors);
    if (document.readyState !== 'loading') prepareAllEditors();
})();


/* =========================================================
   ADMIN ERP V9 - Estética, marca e múltiplos layouts do cardápio
   Mantém a V8 intacta e adiciona controle multi-cliente.
   ========================================================= */
(function adminV9BrandingAndLayouts(){
    const DEFAULT_LOGO = 'https://res.cloudinary.com/de9bsqp2z/image/upload/v1773272375/logo_insta_lanches_instagram_transparente_rk9lxq.png';
    const DEFAULT_NAME = 'InstaLanches';
    const DEFAULT_LAYOUT = 'layout-01';
    const DEFAULT_COLOR = '#d6249f';

    const $ = (id) => document.getElementById(id);
    const qs = (sel, ctx = document) => Array.from(ctx.querySelectorAll(sel));

    window.cardapioBrandingConfig = window.cardapioBrandingConfig || {
        nome_loja: DEFAULT_NAME,
        logo_url: DEFAULT_LOGO,
        layout_cardapio: DEFAULT_LAYOUT,
        cor_primaria: DEFAULT_COLOR
    };

    function safeLayout(value) {
        const v = String(value || DEFAULT_LAYOUT).trim();
        return /^layout-(0[1-9]|1[0-2])$/.test(v) ? v : DEFAULT_LAYOUT;
    }

    function safeColor(value) {
        const v = String(value || DEFAULT_COLOR).trim();
        return /^#[0-9A-Fa-f]{6}$/.test(v) ? v : DEFAULT_COLOR;
    }

    function getInitials(nome) {
        const clean = String(nome || DEFAULT_NAME).trim();
        const parts = clean.split(/\s+/).filter(Boolean);
        return parts.slice(0, 2).map(p => p[0]).join('').toUpperCase() || 'IL';
    }

    function normalizeBranding(config) {
        return {
            nome_loja: String(config?.nome_loja || config?.nome || DEFAULT_NAME).trim() || DEFAULT_NAME,
            logo_url: String(config?.logo_url || DEFAULT_LOGO).trim() || DEFAULT_LOGO,
            layout_cardapio: safeLayout(config?.layout_cardapio || config?.layout || DEFAULT_LAYOUT),
            cor_primaria: safeColor(config?.cor_primaria || config?.tema_cor || DEFAULT_COLOR)
        };
    }

    function setLayoutClass(layout) {
        const safe = safeLayout(layout);
        const classes = Array.from(document.body.classList).filter(c => c.startsWith('layout-cardapio-'));
        classes.forEach(c => document.body.classList.remove(c));
        document.body.classList.add('layout-cardapio-' + safe.replace('layout-', ''));
        document.documentElement.setAttribute('data-cardapio-layout', safe);
        return safe;
    }

    function applyBranding(config) {
        const branding = normalizeBranding(config || window.cardapioBrandingConfig);
        window.cardapioBrandingConfig = branding;

        document.documentElement.style.setProperty('--brand-primary', branding.cor_primaria);
        document.documentElement.style.setProperty('--brand-primary-soft', branding.cor_primaria + '18');
        document.documentElement.style.setProperty('--brand-primary-border', branding.cor_primaria + '33');
        setLayoutClass(branding.layout_cardapio);

        document.title = `${branding.nome_loja} - Cardápio Online`;

        qs('[data-brand-name]').forEach(el => { el.textContent = branding.nome_loja; });
        qs('[data-brand-logo]').forEach(img => {
            if (!img || img.tagName !== 'IMG') return;
            img.src = branding.logo_url || DEFAULT_LOGO;
            img.alt = `Logo ${branding.nome_loja}`;
        });

        const sidebarStatus = $('admin-side-mode');
        if (sidebarStatus) sidebarStatus.textContent = `${branding.nome_loja} direto`;

        const previewNome = $('admin-estetica-preview-nome');
        if (previewNome) previewNome.textContent = branding.nome_loja;

        const previewLogo = $('admin-estetica-preview-logo');
        if (previewLogo) {
            previewLogo.textContent = getInitials(branding.nome_loja);
            previewLogo.style.background = branding.cor_primaria;
        }

        qs('#admin-layout-options .admin-layout-option').forEach(label => {
            const input = label.querySelector('input[type="radio"]');
            label.classList.toggle('admin-layout-selected', !!input && input.value === branding.layout_cardapio);
            if (input) input.checked = input.value === branding.layout_cardapio;
        });

        return branding;
    }

    window.aplicarEsteticaCardapio = applyBranding;

    async function carregarConfiguracoesSupabaseV9() {
        try {
            const { data, error } = await supabaseClient
                .from('configuracoes')
                .select('*')
            ...
            if (data && data.length > 0) {
                ...
                if (config.modo_manual === true) {
                    configLojaGlob.status = config.aberto ? 'FORCAR_ABERTO' : 'FORCAR_FECHADO';
                } else {
                    configLojaGlob.status = 'AUTO';
                }

                configLojaGlob.mostrarBanner = config.mostrar_banner === true;
                configLojaGlob.textoBanner = config.texto_banner || '';

                window.cardapioBrandingConfig = normalizeBranding(config);
                applyBranding(window.cardapioBrandingConfig);

                safeStorage.setItem('InstaLanches_ConfigLoja', JSON.stringify(configLojaGlob));
                safeStorage.setItem('InstaLanches_Branding', JSON.stringify(window.cardapioBrandingConfig));

                statusLoja = calcularStatusLoja();
                aplicarStatusLojaInterface();
            } else {
                const cachedBranding = safeStorage.getItem('InstaLanches_Branding');
                if (cachedBranding) {
                    try { applyBranding(JSON.parse(cachedBranding)); } catch(e) { applyBranding(window.cardapioBrandingConfig); }
                } else {
                    applyBranding(window.cardapioBrandingConfig);
                }
            }
    ...
    window.carregarConfiguracoesSupabase = carregarConfiguracoesSupabaseV9;
    try { carregarConfiguracoesSupabase = carregarConfiguracoesSupabaseV9; } catch(e) {}

    function populateEsteticaForm() {
        const b = normalizeBranding(window.cardapioBrandingConfig);
        const nome = $('admin-estetica-nome-loja');
        const logo = $('admin-estetica-logo-url');
        const cor = $('admin-estetica-cor');
        const preview = $('admin-estetica-logo-preview');
        const file = $('admin-estetica-logo-file');

        if (nome) nome.value = b.nome_loja;
        if (logo) logo.value = b.logo_url || '';
        if (cor) cor.value = b.cor_primaria;
        if (preview) preview.src = b.logo_url || DEFAULT_LOGO;
        if (file) file.value = '';

        applyBranding(b);
    }

    window.carregarAdminEstetica = async function() {
        await carregarConfiguracoesSupabaseV9();
        populateEsteticaForm();
    };

    async function getConfigRowId() {
        const { data, error } = await supabaseClient
            .from('configuracoes')
            .select('id')
            .order('id', { ascending: true })
            .limit(1);
        if (error && error.code !== '42P01') throw error;
        return data && data.length ? data[0].id : 1;
    }

    window.salvarEsteticaCardapioAdmin = async function(btn) {
        const original = btn ? btn.innerHTML : '';
        if (btn) {
            btn.disabled = true;
            btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Salvando...';
        ...
        try {
            const file = $('admin-estetica-logo-file')?.files?.[0] || null;
            let logoUrl = $('admin-estetica-logo-url')?.value?.trim() || '';

            if (file) {
                if (!file.type || !file.type.startsWith('image/')) throw new Error('Escolha um arquivo de imagem válido.');
                if (file.size > 3 * 1024 * 1024) throw new Error('A logo precisa ter até 3MB.');
                logoUrl = await uploadImagemAdmin(file, 'logos');
            }

            const selectedLayout = document.querySelector('input[name="admin_layout_cardapio"]:checked')?.value || DEFAULT_LAYOUT;
            const payload = {
                nome_loja: $('admin-estetica-nome-loja')?.value?.trim() || DEFAULT_NAME,
                logo_url: logoUrl || DEFAULT_LOGO,
                layout_cardapio: safeLayout(selectedLayout),
                cor_primaria: safeColor($('admin-estetica-cor')?.value || DEFAULT_COLOR)
            };

            const id = await getConfigRowId();
            let res = await supabaseClient.from('configuracoes').update(payload).eq('id', id);

            if (res.error) {
                res = await supabaseClient.from('configuracoes').insert([{ id, ...payload }]);
            }

            if (res.error) throw res.error;

            window.cardapioBrandingConfig = normalizeBranding(payload);
            applyBranding(window.cardapioBrandingConfig);
            safeStorage.setItem('InstaLanches_Branding', JSON.stringify(window.cardapioBrandingConfig));

            showToast('Estética do cardápio atualizada!');
            await carregarConfiguracoesSupabaseV9();
            populateEsteticaForm();
        } catch (e) {
            showToast('Erro ao salvar estética: ' + (e.message || 'verifique o SQL'), 'error');
        } finally {
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = original;
    ...
    function bindEsteticaEvents() {
        const nome = $('admin-estetica-nome-loja');
        const logoUrl = $('admin-estetica-logo-url');
        const cor = $('admin-estetica-cor');
        const file = $('admin-estetica-logo-file');
        const preview = $('admin-estetica-logo-preview');

        if (nome && !nome.dataset.v9Bound) {
            nome.dataset.v9Bound = 'true';
            nome.addEventListener('input', () => {
                const cfg = { ...window.cardapioBrandingConfig, nome_loja: nome.value || DEFAULT_NAME };
                applyBranding(cfg);
            });
        }

        if (logoUrl && !logoUrl.dataset.v9Bound) {
            logoUrl.dataset.v9Bound = 'true';
            ...
            logoUrl.addEventListener('change', () => {
                const url = logoUrl.value.trim();
                if (url) applyBranding({ ...window.cardapioBrandingConfig, logo_url: url });
            });
        }

        if (cor && !cor.dataset.v9Bound) {
            cor.dataset.v9Bound = 'true';
            cor.addEventListener('input', () => {
                applyBranding({ ...window.cardapioBrandingConfig, cor_primaria: cor.value || DEFAULT_COLOR });
            });
        }

        if (file && !file.dataset.v9Bound) {
            file.dataset.v9Bound = 'true';
            file.addEventListener('change', () => {
                const selected = file.files && file.files[0];
                if (!selected) return;
                if (!selected.type.startsWith('image/')) {
                    file.value = '';
                    return showToast('Selecione uma imagem válida.', 'error');
                }
                const reader = new FileReader();
                reader.onload = () => {
                    if (preview) preview.src = reader.result;
                    qs('[data-brand-logo]').forEach(img => {
                        if (img.id !== 'admin-estetica-logo-preview') img.src = reader.result;
                    });
                };
        ...
        qs('input[name="admin_layout_cardapio"]').forEach(input => {
            if (input.dataset.v9Bound) return;
            input.dataset.v9Bound = 'true';
            input.addEventListener('change', () => {
                const cfg = { ...window.cardapioBrandingConfig, layout_cardapio: input.value };
                applyBranding(cfg);
            });
        });
    }

    const baseAdminSetTitleV9 = window.adminSetTitle || (typeof adminSetTitle === 'function' ? adminSetTitle : null);
    window.adminSetTitle = function(abaId) {
        if (abaId === 'admin-estetica') {
            const el = $('admin-current-title');
            if (el) el.textContent = 'Estética';
            return;
        }
        if (baseAdminSetTitleV9) return baseAdminSetTitleV9.apply(this, arguments);
    };
    try { adminSetTitle = window.adminSetTitle; } catch(e) {}

    const baseMudarAbaV9 = window.mudarAbaAdmin || (typeof mudarAbaAdmin === 'function' ? mudarAbaAdmin : null);
    window.mudarAbaAdmin = function(abaId, btnElement) {
        const result = baseMudarAbaV9 ? baseMudarAbaV9.apply(this, arguments) : undefined;

        if (abaId === 'admin-estetica') {
            window.adminSetTitle('admin-estetica');
            bindEsteticaEvents();
            carregarAdminEstetica();
        }

        return result;
    };
    try { mudarAbaAdmin = window.mudarAbaAdmin; } catch(e) {}

    const baseRefreshV9 = window.adminRefreshTudo || (typeof adminRefreshTudo === 'function' ? adminRefreshTudo : null);
    window.adminRefreshTudo = async function() {
        if (baseRefreshV9) await baseRefreshV9.apply(this, arguments);
        await carregarConfiguracoesSupabaseV9();
        const active = document.querySelector('.admin-tab-content.block')?.id || '';
        if (active === 'admin-estetica') {
            populateEsteticaForm();
            bindEsteticaEvents();
        }
    };
    try { adminRefreshTudo = window.adminRefreshTudo; } catch(e) {}
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        bindEsteticaEvents();
        setTimeout(bindEsteticaEvents, 300);
    });
...
/* =========================================================
   ADMIN/PÚBLICO V10 - estabilidade desktop + reaplicação de layouts
   ========================================================= */
(function adminV10DesktopAndLayouts(){
    function ensureEditorScrimIsLayer() {
        var panel = document.getElementById('modal-admin-panel');
        var scrim = document.getElementById('admin-editor-scrim');
        if (!panel || !scrim) return;

        // Garante que o scrim nunca roube coluna do grid desktop.
        scrim.setAttribute('aria-hidden', 'true');
        scrim.style.position = 'fixed';
        scrim.style.display = panel.classList.contains('admin-editor-open') ? '' : 'none';

        if (!scrim.dataset.v10Bound) {
            scrim.dataset.v10Bound = 'true';
            scrim.addEventListener('click', function(ev){
                ev.preventDefault();
                ev.stopPropagation();
                if (typeof window.adminCloseCurrentEditor === 'function') {
                    window.adminCloseCurrentEditor(ev);
                }
            }, true);
        }
    }
    ...
    function reapplyPublicLayout() {
        try {
            if (typeof window.aplicarEsteticaCardapio === 'function') {
                window.aplicarEsteticaCardapio(window.cardapioBrandingConfig || {});
            }
        } catch(e) {}
    }

    var baseMontarInterface = window.montarInterface || (typeof montarInterface === 'function' ? montarInterface : null);
    if (baseMontarInterface && !baseMontarInterface.__v10Wrapped) {
        var wrappedMontar = function() {
            var result = baseMontarInterface.apply(this, arguments);
            setTimeout(reapplyPublicLayout, 0);
            return result;
        };
        wrappedMontar.__v10Wrapped = true;
        window.montarInterface = wrappedMontar;
        try { montarInterface = wrappedMontar; } catch(e) {}
    }

    var baseApply = window.aplicarEsteticaCardapio;
    if (baseApply && !baseApply.__v10Wrapped) {
        var wrappedApply = function(config) {
            var result = baseApply.apply(this, arguments);
            document.dispatchEvent(new CustomEvent('cardapio:layout-aplicado', { detail: window.cardapioBrandingConfig || config || {} }));
            return result;
        };
        wrappedApply.__v10Wrapped = true;
        window.aplicarEsteticaCardapio = wrappedApply;
    }

    document.addEventListener('DOMContentLoaded', function(){
        ensureEditorScrimIsLayer();
        reapplyPublicLayout();

        var panel = document.getElementById('modal-admin-panel');
        if (panel && window.MutationObserver) {
            var obs = new MutationObserver(syncScrimVisibility);
...
/* =========================================================
   V12 - Temas inteligentes: cabeçalho + carrinho por layout
   ========================================================= */
(function themeHeaderAndCartV12(){
    const UI_BY_LAYOUT = {
        'layout-01': { header: ['theme-header-center'], cart: 'theme-cart-classic', label: 'Ver pedido' },
        'layout-02': { header: ['theme-header-logo-left','theme-header-compact','theme-header-with-user'], cart: 'theme-cart-compact', label: 'Sacola' },
        'layout-03': { header: ['theme-header-logo-left','theme-header-with-user'], cart: 'theme-cart-media', label: 'Ver pedido' },
        'layout-04': { header: ['theme-header-logo-left','theme-header-with-search','theme-header-with-user','theme-search-primary'], cart: 'theme-cart-app', label: 'Continuar' },
        'layout-05': { header: ['theme-header-center','theme-header-compact'], cart: 'theme-cart-minimal', label: 'Pedido' },
        'layout-06': { header: ['theme-header-logo-left','theme-header-with-user'], cart: 'theme-cart-catalog', label: 'Adicionar' },
        'layout-07': { header: ['theme-header-logo-left','theme-header-floating','theme-header-with-search','theme-header-with-user'], cart: 'theme-cart-neon', label: 'Finalizar' },
        'layout-08': { header: ['theme-header-logo-left','theme-header-compact','theme-header-with-search','theme-search-primary'], cart: 'theme-cart-executive', label: 'Resumo' },
        'layout-09': { header: ['theme-header-center','theme-header-floating','theme-header-with-user'], cart: 'theme-cart-glow', label: 'Carrinho' },
        'layout-10': { header: ['theme-header-logo-left','theme-header-with-user'], cart: 'theme-cart-bistro', label: 'Seu pedido' },
        'layout-11': { header: ['theme-header-logo-left','theme-header-compact','theme-header-with-user'], cart: 'theme-cart-direct', label: 'Comprar' },
        'layout-12': { header: ['theme-header-logo-left','theme-header-floating','theme-header-with-search','theme-header-with-user','theme-search-primary'], cart: 'theme-cart-premium', label: 'Finalizar' }
    };

    const MODE_CLASSES = [
        'theme-header-center',
        'theme-header-logo-left',
        'theme-header-compact',
        'theme-header-floating',
        'theme-header-with-search',
        'theme-header-with-user',
        'theme-search-primary',
        'theme-cart-classic',
        'theme-cart-compact',
        'theme-cart-media',
        'theme-cart-app',
    ...
    function currentLayout(){
        const fromWindow = window.cardapioBrandingConfig && window.cardapioBrandingConfig.layout_cardapio;
        const fromHtml = document.documentElement.getAttribute('data-cardapio-layout');
        const raw = fromWindow || fromHtml || 'layout-01';
        return /^layout-(0[1-9]|1[0-2])$/.test(String(raw)) ? String(raw) : 'layout-01';
    }

    function ensureHeaderTools(){
        const header = document.getElementById('main-header');
        const row = header && header.querySelector(':scope > div:first-child .max-w-5xl');
        if (!row) return null;

        let tools = document.getElementById('theme-header-tools');
        if (!tools) {
            tools = document.createElement('div');
            tools.id = 'theme-header-tools';
            tools.className = 'theme-header-tools';
            tools.innerHTML = `
                <label class="theme-header-search" aria-label="Busca rápida no cardápio">
                    <i class="fas fa-magnifying-glass"></i>
                    <input id="theme-header-search-input" type="search" autocomplete="off" placeholder="Buscar no cardápio">
                </label>
                <button type="button" class="theme-header-user" aria-label="Acompanhar pedido">
                    <i class="fas fa-user-check"></i>
                </button>
            `;
            const bell = document.getElementById('btn-rastreio-header');
            row.insertBefore(tools, bell || null);
        }

        const miniSearch = document.getElementById('theme-header-search-input');
        const mainSearch = document.getElementById('busca-cardapio');

        if (miniSearch && !miniSearch.dataset.v12Bound) {
            miniSearch.dataset.v12Bound = 'true';
            miniSearch.addEventListener('input', function(){
                const target = document.getElementById('busca-cardapio');
                if (target && target.value !== miniSearch.value) {
                    target.value = miniSearch.value;
                    if (typeof window.filtrarCardapioCliente === 'function') window.filtrarCardapioCliente();
                    else if (typeof filtrarCardapioCliente === 'function') filtrarCardapioCliente();
                }
            });
            miniSearch.addEventListener('keydown', function(ev){
                if (ev.key === 'Escape') {
                    miniSearch.value = '';
                    const target = document.getElementById('busca-cardapio');
        ...
        if (mainSearch && miniSearch && !mainSearch.dataset.v12MirrorBound) {
            mainSearch.dataset.v12MirrorBound = 'true';
            mainSearch.addEventListener('input', function(){
                if (miniSearch.value !== mainSearch.value) miniSearch.value = mainSearch.value;
            });
        }

        const userBtn = tools.querySelector('.theme-header-user');
        if (userBtn && !userBtn.dataset.v12Bound) {
            userBtn.dataset.v12Bound = 'true';
            userBtn.addEventListener('click', function(){
                if (typeof window.abrirRastreio === 'function') window.abrirRastreio();
                else if (typeof abrirRastreio === 'function') abrirRastreio();
            });
        }
    ...
    function applyThemeUi(){
        ...
        const ui = UI_BY_LAYOUT[layout] || UI_BY_LAYOUT['layout-01'];

        MODE_CLASSES.forEach(c => document.body.classList.remove(c));
        (ui.header || []).forEach(c => document.body.classList.add(c));
        document.body.classList.add(ui.cart || 'theme-cart-classic');
        document.body.dataset.themeCartMode = (ui.cart || 'theme-cart-classic').replace('theme-cart-', '');
        document.body.dataset.themeHeaderMode = (ui.header || []).join(' ');

        const label = document.getElementById('txt-ver-pedido-btn');
        if (label && ui.label) label.textContent = ui.label;

        const miniSearch = document.getElementById('theme-header-search-input');
        const mainSearch = document.getElementById('busca-cardapio');
        if (miniSearch && mainSearch && miniSearch.value !== mainSearch.value) {
            miniSearch.value = mainSearch.value || '';
        }
    }

    function pulseCartOnCount(){
        const cartBtn = document.getElementById('cart-draggable-btn');
        const count = document.getElementById('cart-count');
        if (!cartBtn || !count || count.dataset.v12CountBound) return;
        count.dataset.v12CountBound = 'true';
        let last = count.textContent;
        const pulse = () => {
            const next = count.textContent;
            if (next !== last) {
                last = next;
                cartBtn.classList.remove('cart-just-updated');
                void cartBtn.offsetWidth;
                cartBtn.classList.add('cart-just-updated');
                window.setTimeout(() => cartBtn.classList.remove('cart-just-updated'), 520);
            }
        };
        if (window.MutationObserver) {
            new MutationObserver(pulse).observe(count, { childList: true, characterData: true, subtree: true });
        }
    }

    const baseApply = window.aplicarEsteticaCardapio;
    if (baseApply && !baseApply.__v12Wrapped) {
        const wrapped = function(){
            const result = baseApply.apply(this, arguments);
            window.setTimeout(applyThemeUi, 0);
            window.setTimeout(applyThemeUi, 120);
            return result;
        };
        wrapped.__v12Wrapped = true;
        window.aplicarEsteticaCardapio = wrapped;
    }

    document.addEventListener('cardapio:layout-aplicado', function(){
        applyThemeUi();
        pulseCartOnCount();
    });
    ...
    document.addEventListener('DOMContentLoaded', function(){
        applyThemeUi();
        pulseCartOnCount();
        setTimeout(applyThemeUi, 250);
        setTimeout(pulseCartOnCount, 250);
    });
    ...
    window.aplicarTemaHeaderCarrinhoV12 = applyThemeUi;
})();
...
/* =========================================================
   V13 - Checkout fullscreen profissional
   Camada comportamental sem quebrar IDs/funções existentes.
   ========================================================= */
(function(){
    function q(id){ return document.getElementById(id); }
    function hasValue(id){ const el = q(id); return !!(el && String(el.value || '').trim()); }
    function cartModalOpen(){
        const modal = q('cart-modal');
        return !!(modal && modal.classList.contains('show'));
    }
    function cartCount(){
        try {
            return (Array.isArray(carrinho) ? carrinho : []).reduce((sum, item) => sum + Number(item.quantidade || 0), 0);
        } catch(e) {
            const count = q('cart-count');
            return Number(count && count.textContent ? count.textContent : 0) || 0;
        }
    }
    function deliveryOk(){
        const tipo = q('tipo-pedido') ? q('tipo-pedido').value : '';
        if (tipo === 'retirada') return true;
        if (tipo !== 'entrega') return false;
        const bairro = q('bairro-select');
        const bairroOk = bairro && bairro.selectedIndex > 0;
        return !!(bairroOk && hasValue('rua-input') && hasValue('numero-input'));
    }
    function paymentOk(){
        const pagamento = q('pagamento');
        return !!(pagamento && pagamento.value);
    }
    function customerOk(){
        return hasValue('nome-cliente') && hasValue('whatsapp-cliente');
    }
    function selectedType(){
        const tipo = q('tipo-pedido');
    ...
    function updateCheckoutUX(){
        const count = cartCount();
        const isOpen = cartModalOpen();
        const type = selectedType();
        const dOk = deliveryOk();
        const pOk = paymentOk();
        const cOk = customerOk();
        const ready = !!(isOpen && count > 0 && cOk && type && dOk && pOk);

        const countLabel = q('checkout-summary-count');
        if (countLabel) countLabel.textContent = count === 1 ? '1 item' : `${count} itens`;

        const progressText = q('checkout-progress-itens-text');
        if (progressText) progressText.textContent = count === 1 ? '1 item' : `${count} itens`;

        setStep('checkout-step-itens', count > 0 ? 'done' : 'active');
        setStep('checkout-step-entrega', dOk && cOk && type ? 'done' : (count > 0 ? 'active' : ''));
        setStep('checkout-step-pagamento', pOk ? 'done' : (dOk && cOk && type ? 'active' : ''));

        const pill = q('checkout-status-pill');
        const pillLabel = q('checkout-status-label');
        if (pill) pill.classList.toggle('ready', ready);
        if (pillLabel) {
            if (!count) pillLabel.textContent = 'Carrinho vazio';
            else if (!cOk) pillLabel.textContent = 'Informe seus dados';
            else if (!type || !dOk) pillLabel.textContent = 'Complete a entrega';
            else if (!pOk) pillLabel.textContent = 'Escolha o pagamento';
            else pillLabel.textContent = 'Pronto para enviar';
        }

        const bar = q('checkout-floating-bar');
        const btn = q('btn-finalizar');
        const txt = q('txt-finalizar-btn');
        ...
        if (btn) {
            btn.classList.toggle('checkout-disabled', !ready);
            btn.setAttribute('aria-disabled', ready ? 'false' : 'true');
            if (ready) btn.removeAttribute('title');
            else btn.setAttribute('title', 'Complete os dados para finalizar o pedido');
        }

        if (txt) txt.textContent = ready ? 'Finalizar' : 'Complete o pedido';
    }

    const oldValidar = window.validarCheckout || (typeof validarCheckout === 'function' ? validarCheckout : null);
    if (oldValidar && !oldValidar.__v13Checkout) {
        const wrapped = function(){
            const result = oldValidar.apply(this, arguments);
            window.setTimeout(updateCheckoutUX, 0);
            return result;
        };
        wrapped.__v13Checkout = true;
        window.validarCheckout = wrapped;
        try { validarCheckout = wrapped; } catch(e) {}
    }

    ['mudarTipoPedido', 'toggleTroco', 'calcularTotal', 'atualizarCarrinho', 'abrirModal', 'fecharModal'].forEach(function(fnName){
        const original = window[fnName] || (typeof globalThis[fnName] === 'function' ? globalThis[fnName] : null);
        if (!original || original.__v13Checkout) return;
        const wrapped = function(){
            const result = original.apply(this, arguments);
            window.setTimeout(updateCheckoutUX, 0);
            window.setTimeout(updateCheckoutUX, 80);
            return result;
        };
        wrapped.__v13Checkout = true;
    ...
    function bindCheckoutFields(){
        ...
        if (!form || form.dataset.v13Bound) return;
        form.dataset.v13Bound = 'true';
    ...
    document.addEventListener('DOMContentLoaded', function(){
        bindCheckoutFields();
        updateCheckoutUX();
        setTimeout(updateCheckoutUX, 250);
    });

    document.addEventListener('click', function(ev){
        if (ev.target && ev.target.closest && ev.target.closest('#cart-modal')) {
            setTimeout(updateCheckoutUX, 0);
        }
    }, true);
    ...
    window.atualizarCheckoutUXV13 = updateCheckoutUX;
})();


/* =========================================================
   V14 - Ajuste cirúrgico: ação contextual mobile no cabeçalho do admin
   ========================================================= */
(function adminMobileHeaderActionsV14(){
    const ACTIONS = {
        'admin-dashboard': { label: 'Atualizar', icon: 'fas fa-rotate-right', run: function(){ return callIfExists('adminCarregarDashboard'); } },
        'admin-pedidos': { label: 'Atualizar', icon: 'fas fa-rotate-right', run: function(){ return callIfExists('carregarPedidosMobile'); } },
        'admin-cardapio': { label: 'Novo', icon: 'fas fa-plus', run: function(){ return callIfExists('adminAbrirFormularioProdutoNovo'); } },
        'admin-promocoes': { label: 'Nova', icon: 'fas fa-plus', run: function(){ return callIfExists('adminAbrirFormularioPromocaoNovo'); } },
        'admin-taxas': { label: 'Novo', icon: 'fas fa-plus', run: function(){ return callIfExists('adminAbrirFormularioTaxa'); } }
    };
    ...
    function activeTabId(){
        const activeContent = document.querySelector('#modal-admin-panel .admin-tab-content.block:not(.hidden)');
        if (activeContent && activeContent.id) return activeContent.id;

        const activeBtn = document.querySelector('#modal-admin-panel .admin-tab-btn.admin-tab-active, #modal-admin-panel .admin-tab-btn.active');
        return activeBtn ? activeBtn.getAttribute('data-admin-tab') : 'admin-dashboard';
    }

    function ensureButton(){
        const actions = document.querySelector('#modal-admin-panel .admin-topbar-actions');
        if (!actions) return null;

        let btn = document.getElementById('admin-mobile-context-action');
        if (!btn) {
            btn = document.createElement('button');
            btn.id = 'admin-mobile-context-action';
            btn.type = 'button';
            btn.className = 'admin-soft-btn admin-mobile-context-action hidden';
            btn.setAttribute('aria-label', 'Ação da aba atual');
            btn.innerHTML = '<i class="fas fa-plus"></i><span>Ação</span>';
            actions.insertBefore(btn, actions.firstChild);

            btn.addEventListener('click', function(ev){
                ev.preventDefault();
                ev.stopPropagation();
                const tab = btn.dataset.adminActionTab;
                const action = tab && ACTIONS[tab];
    ...
    function sync(){
        ...
        if (!isMobile() || !action) {
            ...
            btn.dataset.adminActionTab = '';
            return;
        }

        btn.dataset.adminActionTab = tab;
    ...
    const originalMudar = window.mudarAbaAdmin || (typeof mudarAbaAdmin === 'function' ? mudarAbaAdmin : null);
    if (originalMudar && !originalMudar.__v14MobileActions) {
        const wrapped = function(){
            const result = originalMudar.apply(this, arguments);
            setTimeout(sync, 0);
            setTimeout(sync, 120);
            return result;
        };
        wrapped.__v14MobileActions = true;
        window.mudarAbaAdmin = wrapped;
        try { mudarAbaAdmin = wrapped; } catch(e) {}
    }
    ...
    document.addEventListener('click', function(ev){
        if (ev.target && ev.target.closest && ev.target.closest('#modal-admin-panel')) {
            setTimeout(sync, 0);
        }
    }, true);
    ...
    window.adminSyncMobileHeaderActionV14 = sync;
})();
